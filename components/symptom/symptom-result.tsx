    ...
    <div className="space-y-6">
      ...
      <Card className={`${urgency.color} p-6 border-2`}>
        <div className="text-center">
          <div className="text-4xl mb-3">{urgency.icon}</div>
          <h2 className="text-xl font-bold mb-2">{urgency.title}</h2>
          <p className="text-lg font-semibold">{urgency.advice}</p>
        </div>
      </Card>
          ...
          {result.possible_conditions.map((condition, idx) => (
            <Card key={idx} className="p-4">
              <div className="flex items-start justify-between gap-4 mb-2">
                ...
                >
                  {condition.likelihood === 'high' && 'Kemungkinan Tinggi'}
                  {condition.likelihood === 'medium' && 'Kemungkinan Sedang'}
                  {condition.likelihood === 'low' && 'Kemungkinan Rendah'}
                </Badge>
              </div>
      ...
      <Card className="bg-blue-50 border-blue-200 p-4">
        <h3 className="font-semibold text-blue-900 mb-2">🏥 Kapan Harus ke Dokter?</h3>
        <p className="text-blue-900 text-sm">{result.when_to_see_doctor}</p>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {(result.urgency_level === 'high' || result.urgency_level === 'emergency') && (
      ...
      <Card className="bg-gray-50 p-4 border-gray-200">
        <p className="text-sm text-gray-600 text-center">
          Hasil analisis ini hanya panduan awal. Dokter adalah satu-satunya yang dapat memberikan
          diagnosis resmi dan pengobatan yang tepat. Jangan lewatkan pemeriksaan medis jika
          diperlukan.
        </p>
      </Card>
    </div>
